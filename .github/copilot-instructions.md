...
```bash
# Build all binaries
mage build              # kube-inflater → bin/kube-inflater
mage resourceInflater   # kube-resource-inflater → bin/kube-resource-inflater
...
**Core flow for `kube-resource-inflater`:**
1. `cmd/kube-resource-inflater/main.go` parses flags into `config.ResourceInflaterConfig`
2. `inflater.Engine` orchestrates bulk creation: calls `plan.CalculateBatchesPlan` to get exponential batch sizes, then runs each batch through a worker pool
3. Each batch calls `resourcegen.ResourceGenerator.Generate()` per item, then creates via the dynamic Kubernetes client
4. For pod-bearing types (`pods`, `jobs`, `statefulsets`), KWOK fake nodes are auto-provisioned first via `kwok.Provisioner` — no `--kwok` flag needed

**Core flow for `kube-inflater`:**
- Deploys a DaemonSet to `kubemark-incremental-test` namespace where each pod runs N kubemark containers, registering N hollow nodes per scheduled pod
...
```
Register new generators in `internal/resourcegen/registry.go`. The registry maps short type names (e.g. `"configmaps"`) to constructors.
...
### Hollow node naming
Hollow nodes are always named `hollow-node-<n>` (see `internal/naming/naming.go`). The `NextAvailableNodeNumber` helper finds the next available index from the existing node list.
