...
export default function ClusterInfoCard({ info }: Props) {
  const items = [
    ...
    { label: 'PVs', value: info.persistentVolumeCount },
    { label: 'StorageClasses', value: info.storageClassCount },
    { label: 'Ingresses', value: info.ingressCount },
  ]
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {items.map((item) => (
          <div key={item.label} className="text-center">
            <div className="text-xl font-bold">{(item.value ?? 0).toLocaleString()}</div>
