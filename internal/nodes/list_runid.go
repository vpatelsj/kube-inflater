...
func ListKubemarkNodesForRun(ctx context.Context, client *kubernetes.Clientset, runID string) ([]string, int, error) {
    ...
    for i := 0; i < 100; i++ {
        ...
        if err != nil { return names, 0, err }
        for _, n := range lst.Items {
            names = append(names, n.Name)
            for _, cond := range n.Status.Conditions { if cond.Type == corev1.NodeReady && cond.Status == corev1.ConditionTrue { ready++; break } }
        }
        if lst.Continue == "" { break }
        cont = lst.Continue
    }
